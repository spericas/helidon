...
class GrpcReflectionService implements GrpcService {
    ...
    private ServerReflectionResponse processRequest(ServerReflectionRequest req) {
        return switch (req.getMessageRequestCase().getNumber()) {
            ...
            case ServerReflectionRequest.FILE_CONTAINING_SYMBOL_FIELD_NUMBER -> findProtoFile(req);
    ...
    private ServerReflectionResponse findProtoFile(ServerReflectionRequest req) {
        List<GrpcRouting> grpcRoutings = GrpcReflectionFeature.socketGrpcRoutings().get(socket);

        FileDescriptorResponse.Builder builder = FileDescriptorResponse.newBuilder();
        for (GrpcRouting grpcRouting : grpcRoutings) {
            for (GrpcRoute grpcRoute : grpcRouting.routes()) {
                if (req.getFileContainingSymbol().equals(grpcRoute.serviceName())
                        && grpcRoute instanceof GrpcServiceRoute serviceRoute) {
                    GrpcService service = serviceRoute.service();
                    Objects.requireNonNull(service);
                    try {
                        ByteString byteString = ByteString.readFrom(service.protoFile());
                        System.out.println(byteString);
                        builder.addFileDescriptorProto(byteString);
                        return ServerReflectionResponse.newBuilder().setFileDescriptorResponse(builder).build();
                    } catch (IOException e) {
